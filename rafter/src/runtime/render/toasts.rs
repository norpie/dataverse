...
pub fn render_toasts(
    ...
    for (i, (toast, _created_at)) in toasts.iter().enumerate() {
        ...
        let slide_out_progress = if time_until_expiry >= SLIDE_DURATION {
            ...
            ease_in((time_since_expiry.as_secs_f32() / SLIDE_DURATION.as_secs_f32()).min(1.0) 
                    + (1.0 - time_until_expiry.as_secs_f32() / SLIDE_DURATION.as_secs_f32()).max(0.0))
                .min(1.0)
        } else {
        ...
        let effective_height = (toast_height as f32 + TOAST_MARGIN as f32) * (1.0 - slide_out_progress);
        ...
        lines.push(Line::from(vec![
            Span::styled(
                " ● ",
                RatatuiStyle::default().fg(accent_color),
            ),
            Span::styled(
                title,
        ...
        if let Some(body) = &toast.body {
            for line in body.lines() {
                lines.push(Line::from(vec![
                    ...
                    Span::styled(
                        line.to_string(),
                        RatatuiStyle::default().fg(muted_color),
                    ),
                ]));
            }
        }
...
pub fn calculate_toast_removal_time(
    toast_index: usize,
    toasts: &[(Toast, Instant)],
) -> Instant {
