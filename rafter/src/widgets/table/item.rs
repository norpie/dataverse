...
///             0 => page! { text { self.name.clone() } },
///             1 => page! { text { self.email.clone() } },
///             2 => page! { text { self.age.to_string() } },
///             _ => return None,
///         };
...
///     fn render_cell(&self, col_idx: usize, focused: bool, selected: bool) -> Option<Node> {
///         match col_idx {
///             0 => Some(page! { text (bold) { self.name.clone() } }),
///             1 => Some(page! { text (fg: muted) { self.email.clone() } }),
///             2 => Some(page! {
...
pub trait TableRow: Send + Sync + Clone + 'static {
    ...
    ///
    /// Uses the same purple color scheme as List/Tree widgets.
