...
pub trait App: Clone + Send + Sync + 'static {
    ...
    ///
    /// Returns `Some(new_instance)` if the app supports restart (has `on_panic = Restart`),
    /// or `None` if restart is not supported.
    ///
    /// This method is generated by the `#[app]` macro when `on_panic = Restart` is set.
    /// It requires the app to implement `Default`.
    fn restart(&self, _instance_id: super::InstanceId) -> Option<Box<dyn super::AnyAppInstance>> {
