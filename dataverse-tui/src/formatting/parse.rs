//! Value parsing utilities for converting strings to Dataverse Values.
...
/// This creates an `EntityBinding` suitable for write operations.
pub fn string_to_value(
    ...
    match attr_type {
        ...
        AttributeType::Boolean => {
            let b = parse_bool(s).ok_or_else(|| ParseError::InvalidBool(s.to_string()))?;
            Ok(Value::Bool(b))
        }

        AttributeType::DateTime => {
            let dt = parse_datetime(s).ok_or_else(|| ParseError::InvalidDateTime(s.to_string()))?;
        ...
        AttributeType::Uniqueidentifier => {
            let id: Uuid = s
                .parse()
                .map_err(|_| ParseError::InvalidUuid(s.to_string()))?;
            Ok(Value::Guid(id))
        }

        AttributeType::Lookup | AttributeType::Customer | AttributeType::Owner => {
            let target = target_set_name.ok_or(ParseError::LookupMissingTarget)?;
            let id: Uuid = s
                .parse()
                .map_err(|_| ParseError::InvalidUuid(s.to_string()))?;
        ...
        AttributeType::MultiSelectPicklist => {
            ...
            let values = values.map_err(|_| ParseError::InvalidInt(s.to_string()))?;
...
/// Parse a string to a Value for filter conditions.
///
/// Unlike `string_to_value`, this:
/// - Returns an error for empty strings (filters require a value)
/// - Returns `Value::Guid` for lookups (for comparison, not binding)
pub fn parse_filter_value(s: &str, attr_type: Option<AttributeType>) -> Result<Value, ParseError> {
    let s = s.trim();
    if s.is_empty() {
    ...
    match attr_type {
        Some(AttributeType::String | AttributeType::Memo) => Ok(Value::String(s.to_string())),
